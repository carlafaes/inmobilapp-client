...
export default function Mail(){
    ...
    const handleSubmit=e=>{
        ...
        sendMail({userEmail,userName}).then(data=>{
            if(data.err){
    ...
    return(
        ...
        <form id='form-sus' className='form-mail' onSubmit={handleSubmit}>
            <label className='label-email-form'>
                ...
                <input className='input-mail' type='text' placeholder='nombre' required value={userName} onChange={handleChange('userName')} />
            </label>
            <label className='label-email-form'>
                Correo electronico:<br/>
                <input className='input-mail' type='email' placeholder='correo electronico' required value={userEmail} onChange={handleChange('userEmail')} />
            </label>
            <button className='btn_sus' type='submit'>Enviar</button>
        </form>
        {
            status ? <div><h2 className='label-email-form'>Enviado exitosamente!</h2></div>: <div></div>
